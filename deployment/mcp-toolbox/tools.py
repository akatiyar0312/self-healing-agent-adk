...
sonar_tool = MCPToolset(
    connection_params=StreamableHTTPConnectionParams(
        url="https://api.githubcopilot.com/mcp/",
        headers={
            "Authorization": "Bearer " + os.getenv("SONARQUBE_TOKEN"),
        },
    ),
    # Read only tools
    tool_filter=[
...
playwright_tool = MCPToolset(
    connection_params=StreamableHTTPConnectionParams(
        url="http://localhost:5001",  # Or wherever MCP is hosted
        headers={},
    ),
    tool_filter=["run_tests", "get_test_results"]
)
